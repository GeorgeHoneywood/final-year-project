...
Therefore, to render at the correct resolution you must use the \texttt{window.devicePixelRatio} property, to determine how many physical pixels there are per CSS pixel. You can then use this to set the \texttt{canvas.width} and \texttt{height} to be the same as the screens' physical pixel height and width. As you are now rendering more pixels, you'll then need scale the canvas back down to the size it is rendering at, which will remove the blur.
