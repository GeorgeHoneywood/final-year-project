...
Another option is to use the Cache API\@. The Cache API is part of the Service Worker specification~\cite{w3c-service-workers-caches}~\cite{mdn-service-workers}, and has good cross-browser support. It has a simpler interface that should work for the application --- a pair of a request and a response object are stored, and you get a cached version of the response by providing a similar request. 
